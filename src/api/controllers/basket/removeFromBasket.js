...
export const removeFromBasket = async (req, res) => {
  try {
    ...
    if (productInBasket) {
      ...
      // Проверяю, остались ли еще товары в корзине
      const remainingItems = await Basket.findAll({
        where: { id_user: userId },
      })

      if (remainingItems.length === 0) {
        return res.status(200).json({ message: "Корзина пуста" }) // Сообщаю, что корзина пустая
      }

      return res
        .status(200)
        .json({ message: "Товар успешно удалён из корзины" })
    } else {
